...
Strategy.prototype._challenge = function(code, desc, uri) {
  var challengeParts = [];
  if (this._realm) {
    challengeParts.push('realm="' + this._realm + '"');
  }
  if (this._scope) {
    challengeParts.push('scope="' + this._scope.join(' ') + '"');
  }
  if (code) {
    challengeParts.push('error="' + code + '"');
  }
  if (desc && desc.length) {
    challengeParts.push('error_description="' + desc + '"');
  }
  if (uri && uri.length) {
    challengeParts.push('error_uri="' + uri + '"');
  }
